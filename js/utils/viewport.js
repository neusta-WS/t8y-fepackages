define(['module', 'jquery', 'underscore', 'backbone', '../types/Point', '../types/Size', '../types/Bounds', '../types/Scroll', './animationFrame', './random'], function(module, $, _, Backbone, Point, Size, Bounds, Scroll, animationFrame, random) {
    ...
    return new (Backbone.Model.extend({
        ...
        initialize: function() {
            ...
            if(this.get('frame').addEventListener) {
                this.get('frame').addEventListener('resize', _.throttle(function() {
                    resizeUpdate(scope, scope.EVENT_TYPES.RESIZE);
                }, module.config().resizeInterval || scope.DEFAULT.RESIZE_INTERVAL), false);

                this.get('frame').addEventListener('scroll', function(e) {
                    scope.update(scope.EVENT_TYPES.SCROLL);
                }, false);
            } else {
                this.get('frame').attachEvent('resize', _.throttle(function() {
                    resizeUpdate(scope, scope.EVENT_TYPES.RESIZE);
                }, module.config().resizeInterval || scope.DEFAULT.RESIZE_INTERVAL));

                this.get('frame').attachEvent('scroll', function(e) {
                    scope.update(scope.EVENT_TYPES.SCROLL);
                });
            }
        ...
        register: function(callback) {
            return this.get('callbacks').add({callback: callback});
        },
